...
def main(args):
    ...
    for class_idx in trange(class_labels):
        remaining_samples = args.num_samples_per_class
        while remaining_samples > 0:
            current_batch_size = min(remaining_samples, args.batch_size)
            c_indices = torch.full((current_batch_size,), class_idx, device=device)
            ...
            for i, sample in enumerate(samples):
                save_image(sample, os.path.join(args.save_path, f"{i + total_images:6d}.png"), nrow=1, padding=0)
...
"""
python3 generation_single_gpu.py --vq-ckpt pretrained_weights/vq_ds16_c2i.pt --gpt-ckpt pretrained_weights/c2i_3B_384.pt --gpt-model GPT-3B --image-size 256
"""